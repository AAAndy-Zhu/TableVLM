...
ANSWERS_FILE="${ANSWERS_FILE:-${ROOT_DIR}/outputs/${MODEL_TYPE}/mmtab_${MODE}.jsonl}"

if [[ -z "${MODEL_PATH}" ]]; then
  echo "Please set MODEL_PATH, for example:"
  echo "MODEL_PATH=/path/to/model MODEL_TYPE=qwen3vl MODE=cot bash $0"
  exit 1
fi

python "${SRC_DIR}/eval_mmtab_vllm_da_cot.py" \
